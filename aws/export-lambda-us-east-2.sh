...
# Ensure the 'lambda-us-east-2' directory exists
mkdir -p lambda-us-east-2
...
echo "$function_list" | jq -r '.Functions[].FunctionName' | while read -r name; do
    ...
    wget -q -O "./lambda-us-east-2/$name.zip" "$url"
    ...
    mkdir -p "./lambda-us-east-2/$name"

    cd "./lambda-us-east-2/$name"
